...
#if defined(NDEBUG)
# error "Quantum cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xdf7ec8e12463c55b892dc95e680877e022531760c20bfca6c36e98f1501ff0a5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Quantum: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Quantum Signed Message:\n";
...
{
    // Quantum: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 100 * COIN;
            ...
            if(nHeight == 1)
            {
            nSubsidy = 6210526 * COIN;
            }

    // Subsidy is cut in half every 590000 blocks
    nSubsidy >>= (nHeight / 590000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Quantum: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Quantum: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1487177111, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "First Cloud Management Blockchain";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("049403ad8299cd7698085a5c4a2a7c9b0c7d90e1e1562b59cfdbf118fcc66c18cdbbe872363d5267cee36a2ba5a2ed403cfce9c469db3d5409c493b06a8ee25ed6") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1487177111;
        ...
        assert(block.hashMerkleRoot == uint256("0x2fda658631f28e80d61d339c56904f453ef79313989a964e5fe6eaf04fd9b16e"));
...
unsigned char pchMessageStart[4] = { 0xb9, 0x8c, 0x02, 0x52 }; // Quantum: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("QuantumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("QuantumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static QuantumMiner(CWallet *pwallet)
{
    printf("QuantumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("quantum-miner");
    ...
    try { loop {
        ...
        printf("Running QuantumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("QuantumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&QuantumMiner, pwallet));
}
